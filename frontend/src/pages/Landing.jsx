...
const features = [
  ...
  {
    icon: <Server size={28} className="text-[#2E86AB]" />,
...
const trustStats = [
  { icon: <Database size={20} className="text-[#2E86AB]" />, value: '330K+', label: 'Records migrated per run' },
  { icon: <GitBranch size={20} className="text-[#2E86AB]" />, value: '100+', label: 'Object types supported' },
  { icon: <RefreshCw size={20} className="text-[#2E86AB]" />, value: '10s', label: 'Governor limit headroom built in' },
];
...
export default function Landing() {
  return (
    <div className="min-h-screen bg-[#0f1e30] text-white font-sans">
      {/* Nav */}
      <nav className="flex items-center justify-between px-8 py-5 border-b border-white/10">
        <span className="text-xl font-bold tracking-tight text-[#2E86AB]">OrgIQ</span>
        <div className="flex gap-6 text-sm text-white/70 items-center">
          <a href="#features" className="hover:text-white transition">Features</a>
          <a href="#pricing" className="hover:text-white transition">Pricing</a>
          <Link to="/login" className="text-white hover:text-[#2E86AB] transition">Sign in</Link>
        </div>
      </nav>

      {/* Hero */}
      <section className="flex flex-col items-center text-center px-6 pt-24 pb-20">
        <div className="inline-block text-xs font-semibold tracking-widest uppercase text-[#2E86AB] bg-[#2E86AB]/10 px-4 py-1.5 rounded-full mb-6">
          Salesforce Data Migration — Reimagined
        </div>
        <h1 className="text-5xl md:text-6xl font-extrabold leading-tight max-w-3xl mb-6">
          Any Object.{' '}
          <span className="text-[#2E86AB]">Any Cloud.</span>{' '}
          Any Direction.
        </h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-center">
          <Link
            to="/login"
            className="bg-[#2E86AB] hover:bg-[#247496] text-white font-semibold px-8 py-4 rounded-xl text-base transition shadow-lg shadow-[#2E86AB]/30 flex items-center gap-2"
          >
      ...
      {/* Trust stats */}
      <section className="border-y border-white/10 bg-[#1E3A5F]/20 py-10">
      ...
      <section id="features" className="px-6 py-20 max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {features.map((f) => (
            <div key={f.title} className="bg-[#1E3A5F]/40 border border-white/10 rounded-2xl p-6 hover:border-[#2E86AB]/40 transition">
      ...
      {/* Pricing */}
      <section id="pricing" className="px-6 py-20 bg-[#1E3A5F]/20">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-center text-white/30 mb-12 text-xs">Need more than 250K records or a custom SLA? <a href="mailto:abhishekreddyvontela@gmail.com" className="text-[#2E86AB] hover:underline">Talk to us →</a></p>
          <div className="grid md:grid-cols-4 gap-5 items-start">
            {pricing.map((p) => (
              <div
                key={p.name}
                className={`relative bg-[#0f1e30] rounded-2xl p-6 flex flex-col transition ${
                  p.highlight
                    ? 'border-2 border-[#2E86AB] shadow-lg shadow-[#2E86AB]/20'
                    : 'border border-white/10 hover:border-[#2E86AB]/50'
                }`}
              >
                {p.badge && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#2E86AB] text-white text-xs font-semibold px-3 py-1 rounded-full whitespace-nowrap">
                    {p.badge}
                  </div>
                )}
                <div className="text-xs text-white/40 uppercase tracking-widest mb-3">{p.name}</div>
                <div className="text-3xl font-bold text-[#2E86AB] mb-1">
                  {p.price}
                ...
                <ul className="flex-1 space-y-2 mb-6">
                  {p.features.map((feat) => (
                    <li key={feat} className="flex items-start gap-2 text-sm text-white/70">
                      <Check size={13} className="text-[#2E86AB] mt-0.5 shrink-0" />
                      {feat}
                    </li>
                  ))}
                </ul>
                <Link
                  ...
                  className={`text-center text-sm font-medium py-2.5 rounded-lg transition ${
                    p.highlight
                      ? 'bg-[#2E86AB] hover:bg-[#247496] text-white'
                      : 'bg-[#2E86AB]/10 hover:bg-[#2E86AB]/20 text-[#2E86AB]'
                  }`}
                >
                  {p.cta}
                </Link>
          ...
          {/* Enterprise row */}
          <div className="mt-6 border border-white/10 rounded-2xl px-8 py-6 flex flex-col sm:flex-row items-center justify-between gap-4 hover:border-[#2E86AB]/30 transition">
            <div>
